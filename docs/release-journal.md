...
| (belum dirilis) 1.2.0 | kode 19 Sep 2026 | (commit yang memuat baris ini) | — | residu PR-12 (P-592 register repo Go): state `waiting_printer` + `waiting` (WMI Win32_PnPEntity/Win32_Printer, `usbIds` di katalog; apd menunggu sesudah pasang ≤10 mnt, seagull menunggu sebelum unduh/UAC), auto-map peran dua tahap (`mapping` applied/conflict + `previousPrinter`, tidak menimpa), `POST /setup/cancel`, label QR tanpa QRLABEL → `warning: ROLE_UNMAPPED` (tetap cetak ke default; K-1c tetap ditahan). Perilaku CETAK armada tidak berubah. Smoke CI §14–16 | menunggu rilis (pre-release → PC uji pemilik → kedua toko) | — | — |
